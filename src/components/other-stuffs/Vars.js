  ...
  constructor() {
    ...
    this.authenticateUserInput = (dispatch, email_input, password_input) => {
      ...
      if (!isEmailInputValid) {
        this.showNotify(
          dispatch,
          `You need to check your email again! There must be one digit atleast or your providing are probably fake!`,
          this.boringImg
        );
        return false;
      }
      if (!isPasswordInputValid) {
        this.showNotify(
          dispatch,
          `Check your password again! There must be one digit and one uppercase atleast or your password will not strong enough!`,
          this.angryImg
        );
        return false;
      }
    ...
    this.signIn = async (
      ...
      name,
      password,
      isNeedApplySaveDataId = true
    ) => {
      if (this.isUserSignIn()) {
        const { password, name, savesData, current_saveDataId, token } =
          this.getUserInLocal();
    ...
    this.setCycvObjToLocal = (obj) => {
      if (!localStorage.getItem(this.KEY_CYCV_OBJ)) {
        localStorage.setItem(
          this.KEY_CYCV_OBJ,
          JSON.stringify(cycv_objDefault)
        );
      }
    ...
    this.removeSaveDataInSavesData = (savesData, removedSaveDataId) => {
      return produce(savesData, (draft) => {
        let _index = savesData.findIndex(
          (elt) => elt._id === removedSaveDataId
        );
    ...
    this.updateCurrentSavesData = (currentSaveData, savesData, newSaveData) => {
      ...
      const modifiedSavesData = produce(savesData, (draft) => {
        const indexOfCurrentSaveData = savesData.findIndex(
          (elt) => elt._id === currentSaveData._id
        );
        draft[indexOfCurrentSaveData].saveData.homePage =
          modifiedSaveData.homePage;
        draft[indexOfCurrentSaveData].saveData.preference =
          modifiedSaveData.preference;
      });
    ...
    this.findCurrentSaveData = (current_saveDataId, savesData) => {
      ...
      if (savesData && current_saveDataId) {
        currentSaveData = savesData.find(
          (savefile) => savefile._id === current_saveDataId
        );
      }
      if (currentSaveData) {
    ...
    this.isNextCompPosIndexOk = (
      potentialNextPositionIndex,
      indexOfThisComponent,
      higherComponent_length
    ) => {
      ...
      if (
        nextPositionIndex === indexOfThisComponent ||
        nextPositionIndex < 0 ||
        nextPositionIndex >= higherComponent_length
      ) {
        return false;
      }
      return true;
    ...
    this.findIndexOfComponentIn = (higherComponent, CONPONENT_ID) => {
      const searchConponent = higherComponent.find(
        (component) => component._id === CONPONENT_ID
      );
          ...
          return function (
            ...
            return (
              <ContactBlock
                ...
                isHide={isHide}
                dispatch={dispatch}
                indexOfHigherComponent={indexOfHigherComponent}
                className={className}
              />
            );
          };
    ...
    this.showChooseAvatarOption = (
      dispatch,
      content = "Option modal",
      images = [this.phoneImg],
      chooseAction = (imgUrl) => {}
    ) => {
      dispatch({
    ...
    this.showYesNo = (
      dispatch = () => {},
      content = "Yes or No modal",
      yesAction = () => {},
      noAction = () => {},
      image = this.questionImg
    ) => {
      dispatch({
    ...
    //////////////////
    this.handleDrop =
      (higherComponent, currentComponentId, address, dispatch) => (e) => {
        e.preventDefault();
        document.querySelector(`#${currentComponentId}`).style.backgroundColor =
          "inherit";
        const rawData = e.dataTransfer
          .getData(`dragging-block-data`)
          .split("_");
        if (rawData[0].length === 0)
          return this.showNotify(
            dispatch,
            `You can't transfer component like that!`
          );
        ...
        switch (partsArr.length) {
          case 3:
            ...
            const draggingBlockIndex = this.findIndexOfComponentIn(
              higherComponent,
              draggingBlockId
            );
            const higherComponentName = partsArr[1];
            if (
              higherComponentName.toUpperCase() ===
              draggingHigherComponentName.toUpperCase()
            ) {
              this.changeComponentsIndex(dispatch, draggingBlockIndex, address);
            } else {
              this.showNotify(
                dispatch,
                "You can't transfer component like that!",
    ...
    this.handleDragLeave = (componentLeaveId) => (e) => {
      e.preventDefault();
      document.querySelector(`#${componentLeaveId}`).style.backgroundColor =
        "inherit";
    };
    this.handleDragOver = (componentOverId) => (e) => {
      e.preventDefault();
      document.querySelector(`#${componentOverId}`).style.backgroundColor =
        "#e2ffe4";
    };
    this.handleDragStart = (higherComponentName) => (e) => {
      e.dataTransfer.setData(
        `dragging-block-data`,
        e.target.id + "_" + higherComponentName
      );
