...
const EditableText = ({
  ...
  const handleShowEditableText = (e) => {
    (!isShowEditBtn && setShowEditBtn(true)) ||
      (isShowEditBtn && floatBtnContent === EDIT && setShowEditBtn(false));
  };
  ...
  const closeAndSave = () => {
    ...
    setShowEditBtn(false);
    saveClickAction && saveClickAction();
  };
  React.useEffect(() => {
    return () => {
      clearTimeout(timeoutNumber);
  ...
  const handleEnterKeyCode = (e) => {
    (e.keyCode === 13 || e.keyCode === 27) && closeAndSave();
  };
    ...
    >
      {(readOnly && (
        <InputModified
          ...
          onClick={handleShowEditableText}
          zIndex={zIndex || Vars.EDITABLETEXT_TEXT_ZINDEX}
          readOnly
      ...
      {isShowEditBtn && !readOnly && (
        <FloatButton
          zIndex={Vars.EDITABLETEXT_BTN_ZINDEX}
