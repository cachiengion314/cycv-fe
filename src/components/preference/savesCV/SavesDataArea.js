...
const SavesDataArea = ({
  ...
  const handleRemove = (e) => {
    ...
    if (current_saveDataId === this_saveDataId) {
      Vars.showNotify(
        dispatch,
        "You can't not remove this file since you are opening it already!",
        Vars.sadImg
      );
      return;
    }
    Vars.showYesNo(
      dispatch,
      `Do you really want to remove this save file?`,
      () => {
        Vars.showLoading(
          dispatch,
          `Please wait!`,
          async () => {
            let rawData = await Vars.fetchApi(
              Vars.urlRemoveSaveData(this_saveDataId),
              {
                method: "DELETE",
              }
            );
            if (rawData && rawData.messenger === "successfully!") {
              ...
              const updatedSavesData = Vars.removeSaveDataInSavesData(
                savesData,
                this_saveDataId
              );
  ...
  if (!savesData || savesData.length === 0) {
    return <div>There is no save file</div>;
  }
        ...
        return (
          ...
          >
            ...
            <EditableText
              ...
              readOnly
              value="X"
              className={"float-end custom-btn"}
            />
            <EditableText
              width="18%"
              zIndex="2"
