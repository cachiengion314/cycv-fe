...
const SaveCvNavbar = ({
  ...
  const currentSaveData = React.useMemo(
    () => Vars.findCurrentSaveData(current_saveDataId, savesData),
    [current_saveDataId]
  );
  ...
  const savePng = () => {
    ...
    html2canvas(domElement, {
      onclone: (document) => {
        document.getElementById(Vars.TINY_PREFERENCE_BLOCK).style.visibility =
          "hidden";
      },
    }).then((canvas) => {
      Vars.showHomePageButtons(dispatch);
      Vars.closeModal(dispatch);
      const imgUrlData = canvas
        .toDataURL("image/png")
  ...
  const isUserCanSave = () => {
    ...
    if (Vars.isUserHaveCurrentSaveDataId()) {
      if (!Vars.isCurrentSaveDataId(route.querySaveDataId)) {
        Vars.showNotify(
          dispatch,
          `You can't save change to different savefile!`,
          Vars.sadImg
        );
        return false;
      }
  ...
  const handleOpenSavesArea = () => {
    ...
    Vars.showCustomModal(
      dispatch,
      `All of your save files are here`,
      "70%",
      Vars.createModalBody(<SavesDataArea width="100%" className="" />)
    );
  };
  ...
  const handleSaveCvToHarddrive = () => {
    Vars.showYesNo(
      dispatch,
      `Are you sure wanting save your hard work into hard drive right now?`,
      () => {
        Vars.showLoading(
          dispatch,
  ...
  const handleSaveNewCvToCloud = () => {
        ...
        Vars.showLoading(
          ...
          async () => {
            ...
            if (rawData.messenger === "successfully!") {
              ...
              Vars.showNotify(
                dispatch,
                `Add new save file ${rawData.messenger}`
              );
              ...
              const addedSavesData = Vars.addNewSaveDataToSavesData(
                savesData,
                nextSaveData
              );
              Vars.updateSavesDataInStore(dispatch, addedSavesData, _id);
              // save to local
              Vars.saveUserInfoToLocal(
                userId,
                password,
                name,
                addedSavesData,
      ...
      return (
        <Block width={width} className={className} flexbox={true}>
          <Input
            width="75%"
            maxLength="24"
            fontSize={Vars.FONT_SIZE_MD}
            onChange={handleNameChange}
            value={saveName}
          />
          <Button width="25%" fontSize={Vars.FONT_SIZE_MD} onClick={saveAction}>
            Save
          </Button>
        </Block>
      );
    ...
    Vars.showCustomModal(
      ...
      "70%",
      Vars.createModalBody(
        <EnterSaveFileName width="100%" className="" dispatch={dispatch} />
      )
    );
  };
  ...
  const handleSaveCvToCloud = () => {
    ...
    Vars.showLoading(
      ...
      `Please wait...!`,
      async () => {
        const rawData = await Vars.fetchApi(
          Vars.urlUpdateSaveData(current_saveDataId),
          {
            method: "PUT",
            data: saveObj,
          }
        ...
        if (rawData.messenger === "successfully!") {
          ...
          const updatedSavesData = Vars.updateCurrentSavesData(
            currentSaveData,
            savesData,
            { homePage, preference }
          );
  ...
  return (
    <BlockModified width={width} className={className}>
      ...
      <Text fontSize={Vars.FONT_SIZE_SM} className="mb-1">
        {(currentSaveData && currentSaveData.saveData.name) || "unsaved"}
      </Text>
    </BlockModified>
  );
};
