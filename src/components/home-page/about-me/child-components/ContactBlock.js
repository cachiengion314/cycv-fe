...
const ContactBlock = ({
    ...
    Vars.showChooseAvatarOption(
      ...
      (imgUrl) => {
        Vars.editingText(
          dispatch,
          imgUrl,
          `homePage/aboutMe/${indexOfHigherComponent}/childs/${_id}/image`,
          _id
        );
  ...
  const handleFirstContentChange = (e) => {
    Vars.editingText(
      dispatch,
      e.target.value,
      `homePage/aboutMe/${indexOfHigherComponent}/childs/${_id}/firstContent`,
      _id
    );
  };
  const handleSecondContentChange = (e) => {
    Vars.editingText(
      dispatch,
      e.target.value,
      `homePage/aboutMe/${indexOfHigherComponent}/childs/${_id}/secondContent`,
      _id
    );
  };
  const _imgWith = "13%";
  ...
  const calculateSplitSpaceFor = (secondContent) => {
    return secondContent.length < 13
      ? secondContent
      : STR_splitSpace(secondContent);
  };
  ...
  return (
    <BlockModified width="100%" flexbox={true} className={className}>
      <EditableAvatar
        width={_imgWith}
        src={image || Vars.mailImg}
        onClick={handleAvatarSelect}
      />
      <Block width={_firstContentWith} className="align-center">
        <EditableText
          mediaMaxWidth={Vars.MEDIA_MAX_WIDTH}
          width="100%"
          onChange={handleFirstContentChange}
          maxLength="20"
      ...
      <Block width={_secondContentWith}>
        <EditableText
          ...
          editClickAction={handleWhenEdit}
          fontSize={Vars.FONT_SIZE_SM}
          value={_secondContent}
          saveClickAction={handleWhenSave}
          className="contact-block-editable-text"
          maxLength="40"
        />
      </Block>
