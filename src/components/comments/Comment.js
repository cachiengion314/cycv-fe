...
const BlockStyled = styled(Block)`
  @media (max-width: ${(props) =>
      props.mediaMaxWidth ? props.mediaMaxWidth : "600px"}) {
    width: 100%;
  }
`;
...
const Comment = ({ width, dispatch, socket, socketExecutor, className }) => {
  ...
  const handleSendBtn = async (content) => {
    if (!saveDataIdQuery) {
      Vars.showToast(
        dispatch,
        "You cannot comment in an unsaved file! If you want to comment, lets save your file first!",
        Vars.angryImg
      );
      return;
    }
  ...
  return (
    <BlockStyled
      ...
      className={className}
    >
      <Text fontSize={Vars.FONT_SIZE_SM} className="mb-2">
        Lets discuss something about this cv
      </Text>
      {needLoading && <Loading />}
